...
export function Albums({ songs, onPlaySong }: AlbumsProps) {
    ...
    if (selectedAlbum) {
        return (
            <div className="luma-album-detail p-6 min-h-full animate-fade-in pb-8">
                <GlassButton
                    variant="ghost"
                    onClick={handleBack}
                ...
                <div className="luma-album-hero flex items-end gap-8 mb-10">
    ...
    return (
        <div className="p-6 h-full flex flex-col animate-fade-in">
            ...
            <div className="luma-album-grid grid gap-5 overflow-y-auto scrollbar-hidden pb-32">
                {albums.slice(0, visibleCount).map((album, idx) => (
                    <GlassCard
                        key={idx}
                        onClick={() => setSelectedAlbum(album)}
                        role="button" tabIndex={0} aria-label={`Open album ${album.name}`}
